...
class DocumensoDbService {
    ...
    public function __construct() {
        ...
        $this->teamId = 3;
        $this->minioEndpoint = 'http://{{SERVER_IP}}:9001';
    ...
    private function uploadToMinio($pdfPath) {
        ...
        $cmd = sprintf(
            'curl -X PUT -T %s http://minioadmin:minioadmin123@{{SERVER_IP}}:9001/%s/%s',
    ...
    private function getSigningUrl($documentId, $recipientId) {
        ...
        return "http://{{SERVER_IP}}:9000/sign/" . $token;
    }
    ...
    public function downloadSignedDocument($documentId) {
        ...
        $cmd = sprintf(
            'curl -o %s http://minioadmin:minioadmin123@{{SERVER_IP}}:9001/%s/%s',
