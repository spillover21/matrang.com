...
// eIDAS Electronic Signature endpoints
if ($action === 'createSigningRequest') {
    require_once __DIR__ . '/signature_system.php';
    $system = new eIDASSignatureSystem();
    
    $input = json_decode(file_get_contents('php://input'), true);
    $result = $system->createSigningRequest(
        $input['contract_id'],
        $input['buyer_email'],
        $input['buyer_phone'],
        $input['pdf_url']
...
if ($action === 'getSigningRequest') {
    ...
    $token = $_GET['token'] ?? '';
    $result = $system->getSigningRequest($token);
